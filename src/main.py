...
def list_vm_instances():
    try:
        ...
        return {
            "data": simplified_resources,
            "length": len(resources),
            "message": "List of all VM instances in the project.",
        }
    except Exception as e:
        logger.add_error("list_vm_instances(): Failed to retrieve the VM instances.")
        return JSONResponse(
            content={"data": "", "message": "Failed to retrieve the VM instances."},
            status_code=e.code,
        )
...
def list_vm_instances_in_zone(zone: str):
    try:
        logger.add_info(
            f"list_vm_instances_in_zone(zone={zone}): The list_vm_instances_in_zone route is accessed."
        )
        vic = VMInstanceCollector(credentials)
        ...
        return {
            "data": simplified_resources,
            "length": len(resources),
            "message": f"List of all VM instances in the project in {zone}.",
        }
    except Exception as e:
        logger.add_error(
            f"list_vm_instances_in_zone(zone={zone}): Failed to retrieve the VM instances in {zone}."
        )
        return JSONResponse(
            content={
                "data": "",
                "message": f"Failed to retrieve the VM instances in {zone}.",
            },
            status_code=e.code,
        )
...
def get_vm_instance(zone: str, instance_name: str):
    try:
        logger.add_info(
            f"get_vm_instance(zone={zone}, instance_name={instance_name}): The get_vm_instance route is accessed."
        )
        vic = VMInstanceCollector(credentials)
        resource = vic.collect_resource(zone, instance_name)
        return {
            "data": resource.__repr__(),
            "message": "Details of the specific VM instance.",
        }
    except Exception as e:
        logger.add_error(
            f"get_vm_instance(zone={zone}, instance_name={instance_name}): {str(e)}"
        )
        return JSONResponse(
            content={"data": "", "message": "Failed to retrieve the VM instance."},
            status_code=e.code,
        )
